...
public class RoleProgramServiceTest extends BaseModuleContextSensitiveTest {
	...
	private RoleProgramService getProgramAccessControlService() {
	...
	public void deleteRolePrograms_shouldDeleteRoleProgramsForProgramSuccessfully() throws Exception {
		...
		getProgramAccessControlService().deleteRolePrograms(program);
		Assert.assertTrue(getProgramAccessControlService().getRoles(program).isEmpty());
	}
	...
	public void deleteRolePrograms_shouldDeleteRoleProgramsForRoleSuccessfully() throws Exception {
		...
		getProgramAccessControlService().deleteRolePrograms(role);
		Assert.assertNull(getProgramAccessControlService().getRoleProgram(role,
	...
	public void deleteRoleProgram_shouldDeleteRoleProgramForRoleAndProgramSuccessfully() throws Exception {
		...
		Role role = Context.getUserService().getRole("Some Role");
		getProgramAccessControlService().deleteRoleProgram(role, program);
		Assert.assertNull(getProgramAccessControlService().getRoleProgram(role, program));
	}
	...
	public void getRoleProgram_shouldReturnTheRoleProgramWithTheProgramAndRoleGiven() throws Exception {
		Assert.assertNotNull(getProgramAccessControlService().getRoleProgram(Context.getUserService().getRole("Some Role"),
		    Context.getProgramWorkflowService().getProgram(1)));
	}
	...
	public void getRoles_shouldReturnAllRoleProgramsWithTheGivenProgram() throws Exception {
		Assert.assertEquals(2, getProgramAccessControlService().getRoles(Context.getProgramWorkflowService().getProgram(1))
		        .size());
		Assert.assertEquals(4, getProgramAccessControlService().getRoles(Context.getProgramWorkflowService().getProgram(2))
		        .size());
	}
	...
	public void getPrograms_shouldReturnAllProgramsAuthenticatedUserHasViewPrivilegeTo() throws Exception {
		...
		Assert.assertEquals(1, getProgramAccessControlService().getPrograms().size());
		
		Context.logout();
		Context.authenticate("secondaccount", "test");
		Assert.assertEquals(2, getProgramAccessControlService().getPrograms().size());
	...
	public void saveRoleProgram_shouldSaveGivenRoleProgramSuccessfully() throws Exception {
		...
		getProgramAccessControlService().saveRoleProgram(fac);
		
		Assert.assertNotNull(fac.getUuid());
		Assert.assertNotNull(getProgramAccessControlService().getRoleProgram(role, program));
	}
}
